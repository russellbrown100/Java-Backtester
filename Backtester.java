...
class Chart
{
    ...
    public ArrayList indicator_names;
        
    public Chart(int length, int period)
    {
        indicators = new ArrayList();
        indicator_names = new ArrayList();
        record_number = 0;
        next_date = null;
        this.length = length;
        this.period = period;
...
//                    System.out.println();
                    ...
                    for (int i = 1; i < chart.length; i++)
                    {
                        ...
                        for (int i2 = 0; i2 < chart.indicator_names.size(); i2++)
                        {
                            if (chart.indicator_names.get(i2).toString().equals("simple moving average"))
                            {
                                SimpleMovingAverage ob = (SimpleMovingAverage)chart.indicators.get(i2);                            
                                ob.values[i - 1] = ob.values[i];
                            }
                            else if (chart.indicator_names.get(i2).toString().equals("exponential moving average"))
                            {
